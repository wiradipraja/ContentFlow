...
import { Sparkles, Bot, Brain, Terminal, Palette, Cpu, Zap, Globe, Save, Smartphone, Monitor, LogOut, Eye, EyeOff, X, Loader2, Shield } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (!tempKey) { setDetectedProvider(null); return; }
    const trimmed = tempKey.trim();
    if (trimmed.startsWith('AIza')) setDetectedProvider('GOOGLE');
    else if (trimmed.startsWith('sk-ant')) setDetectedProvider('ANTHROPIC');
    else if (trimmed.startsWith('xai-')) setDetectedProvider('GROK');
    else if (trimmed.startsWith('sk-')) {
        if (detectedProvider !== 'DEEPSEEK' && detectedProvider !== 'OPENART') setDetectedProvider('OPENAI');
    } else {
  ...
  const handleAddKey = async () => {
      ...
      if (detectedProvider === 'GOOGLE') localStorage.setItem('gemini_api_key', tempKey.trim());
      if (user) await saveUserSettings(user.uid, { apiKeys: newKeys });
      setTempKey('');
  ...
  const handleRemoveKey = async (provider: AIProvider) => {
      ...
      if (activeProvider === provider) setActiveProvider('GOOGLE');
      if (user) await saveUserSettings(user.uid, { apiKeys: newKeys });
  };

  const handleSaveGlobals = async () => {
      if (user) {
          await saveUserSettings(user.uid, { activeProvider, defaultPlatform });
  ...
  const handleLogout = async () => {
    await signOut(auth);
    setApiKeys({ GOOGLE: '', OPENAI: '', ANTHROPIC: '', DEEPSEEK: '', OPENART: '', GROK: '' });
    localStorage.clear();
  };
  ...
  if (!user) return <AuthPage onLoginSuccess={() => {}} />;
             ...
             <div className="max-w-4xl mx-auto mt-4 pb-10">
                        ...
                        <div className="relative mb-8 group">
                            ...
                            <div className="relative flex bg-dark-900 rounded-xl border border-dark-600 items-center p-2">
                                ...
                                <div className="flex items-center gap-2 pl-2">
                                    ...
                                    <button onClick={handleAddKey} disabled={!tempKey} className={`px-4 py-2 rounded-lg font-bold text-sm transition-all ${tempKey ? 'bg-brand-600 text-white hover:bg-brand-500' : 'bg-dark-700 text-gray-500 cursor-not-allowed'}`}>Connect</button>
                                </div>
                            </div>
                        </div>
                            ...
                            {[
                                ...
                                return (
                                    <div 
                                        ...
                                        className={`relative p-4 rounded-xl border transition-all cursor-pointer group ${isActive ? 'bg-dark-800 border-brand-500 ring-1 ring-brand-500/50' : isConnected ? 'bg-dark-900 border-dark-600' : 'bg-dark-900/50 border-dark-700 opacity-60'}`}
                                    >
                                        <div className="flex justify-between items-start mb-3">
                                            <div className={`p-2 rounded-lg ${provider.bg} ${provider.color}`}><provider.icon size={20} /></div>
                                            {isConnected && <button onClick={(e) => { e.stopPropagation(); handleRemoveKey(provider.id as AIProvider); }} className="p-1 text-gray-600 hover:text-red-400"><X size={14} /></button>}
                                        </div>
                                        <h3 className="font-bold text-white text-sm">{provider.name}</h3>
                                        <p className={`text-xs mt-1 font-medium ${isConnected ? 'text-green-400' : 'text-gray-500'}`}>{isConnected ? 'Connected' : 'Not Configured'}</p>
                                    </div>
                                );
                            })}
                        </div>
                ...
                <div className="bg-dark-800 rounded-2xl border border-dark-700 overflow-hidden mb-6">
                     <div className="p-6 border-b border-dark-700 bg-dark-900/50">
                        <h2 className="text-lg font-semibold text-white flex items-center gap-2"><Globe className="text-blue-400" size={20} /> Application Defaults</h2>
                     </div>
                     <div className="p-6 grid md:grid-cols-2 gap-6">
                        <div>
                            <label className="block text-sm font-medium text-gray-300 mb-3">Default Target Platform</label>
                            <div className="grid grid-cols-2 gap-2">
                                <button onClick={() => setDefaultPlatform('TIKTOK')} className={`flex items-center justify-center gap-2 py-3 rounded-lg border transition-all ${defaultPlatform === 'TIKTOK' ? 'bg-brand-900/40 border-brand-500 text-white' : 'bg-dark-900 border-dark-600 text-gray-400'}`}><Smartphone size={16} /> TikTok (9:16)</button>
                                <button onClick={() => setDefaultPlatform('YOUTUBE')} className={`flex items-center justify-center gap-2 py-3 rounded-lg border transition-all ${defaultPlatform === 'YOUTUBE' ? 'bg-red-900/40 border-red-500 text-white' : 'bg-dark-900 border-dark-600 text-gray-400'}`}><Monitor size={16} /> YouTube (16:9)</button>
                            </div>
                        </div>
                        <div>
                             <label className="block text-sm font-medium text-gray-300 mb-3">Cloud Sync</label>
                             <button onClick={handleSaveGlobals} className="w-full bg-dark-700 hover:bg-dark-600 text-white py-3 rounded-lg border border-dark-600 font-medium transition-colors flex items-center justify-center gap-2"><Save size={16} /> Save to Cloud</button>
                        </div>
                     </div>
                </div>
                <div className="mt-8 text-center text-[10px] text-dark-500 flex items-center justify-center gap-1"><Shield size={12} /> Data is encrypted and stored safely on Firestore Cloud.</div>
             </div>
        );
